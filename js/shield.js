 ...
 * 
 * Q_fluid = m_fluid * Cp_fluid * (t_out - t_in) = Q_rad + Q_conv
 * Q_fluid = Q_R
 * 
 * Q_conv = h_conv * At * ( Tb(tg_in, tg_out) - Tw(t_in, t_out) )
 * Q_conv = m_flue * Cp_flue * (tg_in - tg_out)
 *****************************************************************/
...
const shieldSection = (params) => {
  ...
  logger.warn(`Acp_shld: ${ round( unitConv.m2toft2(Acp) ) } ft`);
  ...
  logger.default(`T_in_sh: ${round(t_in)} K, Tg_sh: ${round(tg_out)} K`)
