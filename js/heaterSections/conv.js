...
const convSection = (params, noLog) => {
  ...
  const duty_conv = (tIn) => Q_fluid(tIn);  // Duty convective sect used for Tw calc
  ...
  const
    ...
    // (Btu/hr-ft2-F) Outside radiation factor // Practical implementation
    hr = (tG_b, tW) => 2.2 *gr(tG_b, tW) *(PL)**.50 *(Apo/Ao)**.75; 
    // (kJ/m²h-°C) effective radiative coff wall tube

  let hc = (tG_b, _tW) => .33 *(kw_flue(tG_b)/Do) *prandtl_flue(tG_b)**(1/3) *
...
const colburnFactor = (reynoldsFlue, parm, m, B) => {
  const
    ...
    // (K) Average fin temp
    Ts = (tB_g, tW) => tB_g + (tW - tB_g) / 
      ( ( Math.exp(1.4142*m*B) + Math.exp(-1.4142*m*B) )/2 );
  parm.Ts = Ts;
 ...
 * Q_conv(t_in,tg_in,tG_out)/(m_fluid*Cp_fluid(t_in,t_out));
