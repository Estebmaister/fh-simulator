...
const convSection = (params, noLog) => {
  ...
  const // Parameters
    ...
    MBL =
      (2 / 3) *
      (params.Width_rad * params.Length_rad * params.Height_rad) ** (1 / 3),
  ...
  const // Process Functions
    ...
    hi = (tB, tW = tB) =>
      ...
      reynolds(tB) ** 0.8 *
      prandtl(tB) ** (1 / 3) *
      (miu_fluid(tB) / miu_fluid(tW)) ** 0.14;
  ...
  const Tw = (tB = Tb(t_out, t_in), tW = tB, tIn = t_in) =>
    (duty_conv(tIn) / At) *
      (Do / Di) *
      (Rfi + 1 / hi(tB, tW) + (Di * Math.log(Do / Di)) / (2 * kw_tube(tW))) +
    tB;

  const gr = (_tB, _tW) => 2.6 * (0.29307107 * cnv_fact), // (Btu/hr-ft2-F) Outside radiation
    // factor //HACK: Find implementation
    hr = (tG_b, tW) => 2.2 * gr(tG_b, tW) * PL ** 0.5 * (Apo / Ao) ** 0.75; // (kJ/m²h-°C) effective radiative coff wall tube

  let hc = (tG_b, _tW) =>
    0.33 *
    (kw_flue(tG_b) / Do) *
    prandtl_flue(tG_b) ** (1 / 3) *
    reynolds_flue(tG_b) ** 0.6; // (kJ/m²h-°C)

  const ho = (tG_b, tW) => 1 / (1 / (hc(tG_b, tW) + hr(tG_b, tW)) + Rfo), // (kJ/m²h-°C) external heat transfer coff
    /** Fin's Efficiency */
    Kw_fin = 1.36 * kw_tube(Tw(Tb(t_in, t_out), Tw(Tb(t_in, t_out)))), // HACK: Find theoretical implementation
    B = L_fin + Th_fin / 2,
    m =
      (ho(Tb(tg_in, tg_out), Tw(Tb(t_in, t_out), Tw(Tb(t_in, t_out)))) /
        (6 * Kw_fin * Th_fin)) **
      0.5,
    x = Math.tanh(m * B) / (m * B),
    y = x * (0.7 + 0.3 * x),
  ...
  hc = (tG_b, tW) =>
    j(tG_b, tW) * Gn * Cp_flue(tG_b) * prandtl_flue(tG_b) ** -0.67; // (kJ/m²h-°C) film heat transfer coff
  ...
  const Q_conv = (tIn, tG_in, tG_out) =>
    Uo(Tb(tG_out, tG_in), Tb(tIn), Tw(Tb(tIn), Tw(Tb(tIn)))) *
    At *
    LMTD_Tin(tIn);
  ...
  return {
    ...
    t_fin_max:
      t_out +
      (Q_conv(t_in, tg_in, tg_out) /
        (At / (params.N_conv / params.Tpr_sh_cnv))) *
        (Do / Di) *
        (Rfi +
          1 / hi(Tb(t_in), Tw(Tb(t_in), Tw(Tb(t_in)))) +
...
const colburnFactor = (reynoldsFlue, parm, m, B) => {
  const C1 = (tB_g) => 0.25 * reynoldsFlue(tB_g) ** -0.35, // Reynolds number correction
    Lf = parm.Lf, // (m) Fins height
    Sf = 1 / parm.Nf - parm.Tf, // (m) Fin spacing
    C3 = 0.35 + 0.65 * Math.exp((-0.25 * Lf) / Sf), // Geometry correction (Solid, staggered pattern)
    ...
    C5 = 0.7 + (0.7 - 0.8 * Math.exp(-0.15 * Nr ** 2)) * Math.exp(-Pl / Pt), // Non-equilateral & row correction
    Df_Do = (2 * parm.Lf + parm.Do_conv) / parm.Do_conv, // (m) Ratio fin's Do per tube's Do
    Ts = (tB_g, tW) =>
      tB_g +
      (tW - tB_g) /
        ((Math.exp(1.4142 * m * B) + Math.exp(-1.4142 * m * B)) / 2); // (K) Average fin temp
  parm.Ts = Ts;
  return (tB_g, tW) =>
    C1(tB_g) * C3 * C5 * Df_Do ** 0.5 * (tB_g / Ts(tB_g, tW)) ** 0.25;
};
