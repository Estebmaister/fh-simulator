function draw(data = [], opts = {}) {
  const svgHeight = window.innerHeight / 1.6,
    svgWidth = window.innerWidth * (1 - 0.03);
...
function innerDraw(
  ...
  const yAxisFontSize =
    innerWidth < 900 ? innerHeight * 0.07 : innerHeight * 0.08;
  const yAxisTickSize = innerWidth < 900 ? "1em" : "1.2em";
  const titleFontSize =
    innerWidth < 900 ? innerWidth * 0.046 : innerWidth * 0.035;
  ...
  switch (yVar) {
    case "co2_emiss":
      ...
      yAxisLabel =
        opts.lang == "es" ? `${yTitle} [t/año]` : `${yTitle} [t/year]`;
      break;
    case "m_fuel":
