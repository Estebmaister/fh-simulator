...
function draw(data = [], opts = {}) {
  const svgHeight = window.innerHeight * (1 - .15),
  ...
  innerDraw(opts, 'rad_tg_out', graphPerRow == 1 ? 0 : svgWidth / 2,
            graphPerRow == 1 ? svgHeight * 0.5 : 0, drawCommonProps);
  innerDraw(opts, 'cnv_tg_out', 0,
            graphPerRow == 1 ? svgHeight * 1 : svgHeight / 2, drawCommonProps);
  innerDraw(opts, 'rad_cnv_dist', graphPerRow == 1 ? 0 : svgWidth / 2,
            graphPerRow == 1 ? svgHeight * 1.5 : svgHeight / 2,
            drawCommonProps);
  innerDraw(opts, 'efficiency', 0, graphPerRow == 1 ? svgHeight * 2 : svgHeight,
            drawCommonProps);
  innerDraw(opts, 'co2_emiss', graphPerRow == 1 ? 0 : svgWidth / 2,
            graphPerRow == 1 ? svgHeight * 2.5 : svgHeight, drawCommonProps);
}
...
function innerDraw(opts, yVar, xDisplace, yDisplace,
  ...
  const yAxisFontSize =
      innerWidth < 900 ? innerHeight * .055 : innerHeight * .06;
  ...
  case 'm_fluid':
    ...
    break;
  default:
    xTitle = opts.lang == "es" ? 'Temperatura de Salida de residuo'
  ...
  case 'cnv_tg_out':
    ...
    break;
  case 'rad_tg_out':
    yTitle = opts.lang == "es" ? 'Temperatura de Arco Radiante'
                               : 'Radiant Flue Temperature';
    yAxisLabel = `${yTitle} [F]`;
    break;
  case 'rad_cnv_dist':
    yTitle = opts.lang == "es" ? 'Absorción de calor (Rad/Conv)'
  ...
  case 'rad_dist':
    ...
    break;
  case 'cnv_dist':
    yTitle = opts.lang == "es" ? 'Distribución Convectiva'
  ...
  const yScale = d3.scaleLinear()
                     .range([ innerHeight, 0 ])
                     .domain([
                       d3.min(data, d => d[yVar] * .998),
                       d3.max(data, d => d[yVar] * 1.002)
                     ])
                     //.domain(yExtent)
                     .nice();
