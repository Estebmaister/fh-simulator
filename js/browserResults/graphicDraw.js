...
function draw(data = [], opts = {}) {
  const svgHeight = window.innerHeight / 1.6,
  ...
  innerDraw(opts, 'co2_emiss', 0, svgHeight * 0,
            {svg, data, margin, innerHeight, innerWidth});
  innerDraw(opts, 'm_fuel', graphPerRow == 1 ? 0 : svgWidth / 2,
            graphPerRow == 1 ? svgHeight * 0.5 : 0,
            {svg, data, margin, innerHeight, innerWidth});
  innerDraw(opts, 'cnv_tg_out', 0,
            graphPerRow == 1 ? svgHeight * 1 : svgHeight / 2,
            {svg, data, margin, innerHeight, innerWidth});
  innerDraw(opts, 'efficiency', graphPerRow == 1 ? 0 : svgWidth / 2,
            graphPerRow == 1 ? svgHeight * 1.5 : svgHeight / 2,
            {svg, data, margin, innerHeight, innerWidth});
  innerDraw(opts, 'rad_dist', 0, graphPerRow == 1 ? svgHeight * 2 : svgHeight,
            {svg, data, margin, innerHeight, innerWidth});
  innerDraw(opts, 'cnv_dist', graphPerRow == 1 ? 0 : svgWidth / 2,
            graphPerRow == 1 ? svgHeight * 2.5 : svgHeight,
            {svg, data, margin, innerHeight, innerWidth});
}

function innerDraw(opts, yVar, xDisplace, yDisplace,
                   {svg, data, margin, innerHeight, innerWidth}) {
  ...
  const yAxisFontSize =
      innerWidth < 900 ? innerHeight * .07 : innerHeight * .08;
  const yAxisTickSize = innerWidth < 900 ? "1em" : "1.2em";
  const titleFontSize =
      innerWidth < 900 ? innerWidth * .046 : innerWidth * .035;

  const xValue = d => d[opts.graphVar];
  let xAxisLabel = '', xTitle;
  switch (opts.graphVar) {
  case 'humidity':
    xTitle = opts.lang == "es" ? 'Humedad' : 'Humidity';
    xAxisLabel = `${xTitle} [%]`;
    break;
  case 'air_excess':
  ...
  default:
    xTitle =
        opts.lang == "es" ? 'Temp. Salida de residuo' : 'Residue Outlet Temp';
    xAxisLabel = 'Temp [F]';
    break;
  }
  ...
  switch (yVar) {
  case 'co2_emiss':
    yTitle = opts.lang == "es" ? 'Emisiones CO2' : 'CO2 Emissions';
    yAxisLabel = opts.lang == "es" ? `${yTitle} [t/año]` : `${yTitle} [t/year]`;
    break;
  case 'm_fuel':
    yTitle = opts.lang == "es" ? 'Flujo de comb.' : 'Fuel mass flow';
    yAxisLabel = `${yTitle} [lb/h]`;
    break;
  case 'efficiency':
    yTitle = opts.lang == "es" ? 'Eficiencia' : 'Efficiency';
    yAxisLabel = `${yTitle} [%]`;
    break;
  case 'cnv_tg_out':
    yTitle = opts.lang == "es" ? 'Temp. Chimenea' : 'Temp stack';
    yAxisLabel = `${yTitle} [F]`;
    break;
  case 'rad_dist':
    yTitle = opts.lang == "es" ? 'Dist. Radiante' : 'Radiant dist.';
    yAxisLabel = `${yTitle} [%]`;
    break;
  case 'cnv_dist':
    yTitle = opts.lang == "es" ? 'Dist. Convectiva' : 'Convective dist.';
    yAxisLabel = `${yTitle} [%]`;
    break;
  default:
    break;
  ...
  const xScale =
      d3.scaleLinear().range([ 0, innerWidth ]).domain(xExtent).nice();
  const yExtent = d3.extent(data, yValue)
  const yScale =
      d3.scaleLinear().range([ innerHeight, 0 ]).domain(yExtent).nice();

  const xAxis = d3.axisBottom(xScale).tickSize(-innerHeight).tickPadding(15);
  const yAxis = d3.axisLeft(yScale).tickSize(-innerWidth).tickPadding(10);

  const graphic = svg.append("g").attr(
      "transform", `translate(${xDisplace + margin.left * 1.25},${
                       yDisplace + margin.top / 2})`)

  const xAxisG = graphic.append('g').call(xAxis).attr(
