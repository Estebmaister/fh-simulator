...
const graphicData = (comb, fuel, opt) => {
  ...
  if (localResult) {
    ...
    if (Object.keys(localResult).length > 5) localStorage.clear();
  }
  if (prevResult) {
    draw(prevResult, opt);
    return;
  ...
  const points = opt.graphPoints,
    range = opt.graphRange;
  let initVar = opt[graphVar] - range / 2;
  if (initVar < 0) initVar = 0;
  savedLogger.info(
    `Var: ${graphVar}, centerValue: ${opt[graphVar]}, range: ${range}, points: ${points}`
  );
  for (let index = 0; index < points; index++) {
    ...
    browserResult[index] = {
      // --- Input vars
      m_fluid:
        unitConv.lb_htoBPD(unitConv.kgtolb(runResult.rad_result.m_fluid)) *
        1e-3,
      ...
      // m_flue:     runResult.shld_result.m_flue ?
      // runResult.shld_result.m_flue : 0,
      m_fuel: runResult.rad_result.m_fuel
        ? unitConv.kgtolb(runResult.rad_result.m_fuel)
        : 0,
      efficiency: runResult.rad_result.eff_thermal_val
        ? runResult.rad_result.eff_thermal_val
        : 0,
      // duty_total: runResult.rad_result.duty_total,
      rad_dist:
        runResult.rad_result["%"] < 1
          ? Math.round(1e5 * runResult.rad_result["%"]) / 1e3
          : 0,
      cnv_dist:
        runResult.conv_result["%"] < 1
          ? Math.round(1e5 * runResult.conv_result["%"]) / 1e3
          : 0,
      // shl_duty:  runResult.shld_result.duty,
      co2_emiss:
        Math.round(
          1e2 *
            runResult.products["CO2"] *
            (44.01 / runResult.flows["fuel_MW"]) *
            runResult.rad_result.m_fuel *
            (1e-3 * 24 * 365)
        ) / 1e2,
    };
  }
